...
class BaseStore(ABC):
    ...
    def store_stats(self):
        ...
        for product in self.products:
            if product.model not in product_summary:
                product_summary[product.model] = {"count": 0, "total_price": 0.0}
            product_summary[product.model]["count"] += 1
            product_summary[product.model]["total_price"] += product.price
        ...
        for model in sorted(product_summary.keys()):
            count = product_summary[model]["count"]
            avg_price = product_summary[model]["total_price"] / count
